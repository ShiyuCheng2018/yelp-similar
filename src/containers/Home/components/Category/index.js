	...
	render() {
		...
		return (
			<div className={"category"}>
				<Slider {...settings}>
					{
						dataSource.map((section, i)=>{
						return <div key={i}>
							{section.map((item, i)=>{
								return <div key={i} className={"category__section"}>
									<img alt={"icon"} src={item.src} className={"category__icon"}/>
									<div className={"category__text"}><span>{item.name}</span></div>
								</div>
							})}
						</div>
						})
